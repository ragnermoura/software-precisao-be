                ...
                const response = {
                    ...
                    planos: result.map(plan => {
                        return {
                            ...
                            descricao: plan.descricao_plano,
...
router.post('/cadastro', (req, res, next) => {
    mysql.getConnection((error, conn) => {
        if (error) { return res.status(500).send({ error: error }) }
        conn.query(
            'INSERT INTO tb002_plano (nome_plano, descricao_plano, valor_plano) VALUES (?,?,?)',
            [
                req.body.plano,
                req.body.descricao,
                req.body.valor,
...
router.get('/:id_plano', (req, res, next) => {
    mysql.getConnection((error, conn) => {
        if (error) { return res.status(500).send({ error: error }) }
        conn.query(
            'SELECT * FROM tb002_plano WHERE id_plano = ?',
            [req.params.id_user],
            (error, result, fields) => {
                if (error) { return res.status(500).send({ error: error }) }
